        ...
        if self.od_bounding_boxes is not None:
            for box in self.od_bounding_boxes:
                if box["confidence"] >= self.THRESHOLD:
                    ...
                    # Convert to Integer
                    pt1_x = int(box["bounding_polygon"]["normalized_vertices"][0]["x"]* self.width)
                    pt1_y = int(box["bounding_polygon"]["normalized_vertices"][0]["y"]* self.height)
                    pt2_x = int(box["bounding_polygon"]["normalized_vertices"][2]["x"]* self.width)
                    pt2_y = int(box["bounding_polygon"]["normalized_vertices"][2]["y"]* self.height)
                    ...
                    cv2.rectangle(im_bb, pt1, pt2, self.label_map[box["name"]], 2)
                    # Plot Label just above the Top Left Point, Set Font, Size, Colour, Thickness
                    cv2.putText(im_bb,box["name"],(pt1_x, pt1_y - 10),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255, 255, 0),2)
